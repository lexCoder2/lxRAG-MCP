...
```json
{
  "memgraphNodes": 2216,  "memgraphRels": 3622,
  "cachedNodes": 448,     "cachedRels": 2250,
  "indexedFiles": 74,     "indexedFunctions": 85,  "indexedClasses": 164,
  "driftDetected": true,
...
### SX1 — SECTION.title Never Populated *(Low)*
...
### SX2 — FUNCTION / CLASS Nodes Missing `path` Property *(Medium)*
...
### SX3 — REFERENCES Edges Not Created for TypeScript `.js` Imports *(High)*  
...
### SX4 — `test_run` Tool Inherits Wrong Node.js from Server Process PATH *(High)*
...
### SX5 — `misc` Community Dominates (77% of Members) *(Medium)*
...
### SX6 — Feature Registry Empty *(Low)*
...
### SX7 — `reflect` Returns 0 Learnings *(Low)*

**Observed:**  
```json
{ "learningsCreated": 0, "insight": "Reflection over 1 episodes: no dominant recurring entities detected." }
```
